...
def run_training(model, data_folds, label_folds, spectrogram_folds):
    ...
    for e in epoch_num:
        ...
        for fold in range(num_folds):
            ...
            model.fit(train_dataset,validation_data=validation_dataset, epochs=10, callbacks=[callback,tensorboard_callback, checkpoint_callback, reducelr_callback]) #define params
...
def objective (params):
    ...
    with mlflow.start_run():
        ...
        model = create_audio_model((128,238), 10, num_transformer_blocks=params['num_transformer_blocks'], num_diffusion_steps=params['num_diffusion_steps'])
        
        model = run_training(params['data_folds'], params['label_folds'], params['spectrogram_folds'])

        test_labels = np.array(params['label_test'])
        test_spect = np.array(params['spectrogram_test'])
        test_data = np.array(params['test_data'])
...
def main():
    ...
    search_space= {
        'num_transformer_blocks': hp.uniform('num_transformer_blocks', 1, 10),
        'num_diffusion_steps': hp.uniform('num_diffusion_steps', 10, 20),
        'data_folds': data_folds,
        'label_folds':label_folds,
        'spectrogram_folds':spectrogram_folds,
        'test_data':data_test,
        'label_test':label_test,
        'spectrogram_test':spectrogram_test
        }
    
    mlflow.set_tracking_uri("http://127.0.0.1:5000") #  connects to a tracking URI.
    mlflow.set_experiment("final-project-experiment")
    best_result = fmin(
        fn=objective,
        space=search_space,
        algo=tpe.suggest,
